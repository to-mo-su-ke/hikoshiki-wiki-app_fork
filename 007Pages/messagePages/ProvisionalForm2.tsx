...
interface PrvisionalFormRepository{
    getSubjectDetail(subjectId: string): Promise<SubjectDetail>; //シラバスから教科名/教授名などを受け取る
    updateReview(subjectId: string, review: QuantitativeSubjectReview, key: string): Promise<void>; //入力されたレビューに関する処理
...
//レビューの合計値を保管する
class PrvisionalFormSumRepository implements PrvisionalFormRepository
{
    async getSubjectDetail(subjectId: string){
        ...
        let newInstructor=instructor.replace(/○[\s\S]*/u,"") //教授名が2名以上いる場合は先頭の1人に限定
        return{
    ...
    async updateReview(subjectId: string, review: QuantitativeSubjectReview, key: string){
        ...
        let reviewDoc = await firestore.doc(`DetailTemplate2_sum/information/${colname}/${name}`); //sとtで参照するコレクションを変更
    ...
    async saveMessageReview(subjectId: string, message: ReviewMessage, review: QuantitativeSubjectReview){
        const subjectdetail=await this.getSubjectDetail(subjectId);
        const messageCollection=firestore.collection(`DetailTemplate2_sum/information/messages`)
...
//レビューのリストを保管
class PrvisionalFormListRepository implements PrvisionalFormRepository
{
    async getSubjectDetail(subjectId: string){
        const subjectRef=`syllabus/${subjectId}`;
        const syllabusDoc=await firestore.doc(subjectRef).get()
    ...
    async updateReview(subjectId: string, review: QuantitativeSubjectReview, key: string){
        const ArrayIncrement = (reviewNumber: number, arrayRating: number[]=[0,0,0,0,0]) => (arrayRating[reviewNumber-1]++, arrayRating); //リストに追加する関数
        //☆1は配列のindex0として保存(配列で保管)
        const subjectdetail=await this.getSubjectDetail(subjectId);
        let name = key=="s"?subjectdetail.nameOfSubject:subjectdetail.nameOfTeacher;
        let colname = key=="s"?"subjectReview":"teacherReview";
        let reviewDoc = await firestore.doc(`DetailTemplate2_list/information/${colname}/${name}`);  //sとtで参照するコレクションを変更
        await firestore.runTransaction(async (transaction)=>{ //transactionの利用でデータの損失を防ぐ
            let reviewSnap=await transaction.get(reviewDoc);
            if(reviewSnap.exists){
                transaction.update(reviewDoc,{
                ...
                }) //それぞれの評価値に対してリストを更新する(documentNumberはリストから求められるため不要)
            }else{
                transaction.set(reviewDoc,{
                ...
                }) //存在しない場合は新しいドキュメントを作成(リスト化必須)
            } 
        })
    }
    async saveMessageReview(subjectId: string, message: ReviewMessage, review: QuantitativeSubjectReview){
        const subjectdetail=await this.getSubjectDetail(subjectId);
        const messageCollection=firestore.collection(`DetailTemplate2_list/information/messages`)
        const messageDoc=messageCollection.doc();
        messageDoc.set
        ({
            content: message.content,
...
const PrvisionalForm=new PrvisionalFormSumRepository() //ここを変更して切り替え
...
export const PrvisionalFormScreen=({navigation,route} :PrvisionalFormScreenProps)=>{
    const submitData=()=>{ //入力をdatabaseに保存する関数(もっときれいに変更したい)
        ...
        PrvisionalForm.updateReview(subjectId,review,"t"); //レビュー, メッセージ(教科・教授)それぞれを保存
        navigation.pop();
        return;
    }
